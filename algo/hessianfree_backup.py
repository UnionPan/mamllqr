...
class HessianFreeMAML:
    ...
    def evaluate_adaptation_curve(self, test_env, max_adaptation_steps: int = 10, num_evaluations: int = 5,
                                baseline_policy: np.ndarray = None) -> Dict[str, Any]:
        """
        Evaluate adaptation curve on a single unseen task.

        Parameters:
        test_env: Unseen test environment
        max_adaptation_steps: Maximum number of adaptation steps to test
        num_evaluations: Number of rollout evaluations per adaptation step
        baseline_policy: Common baseline policy for fair comparison (e.g., random or LQR solution)

        Returns:
        Dict with adaptation curve data:
        - 'adaptation_steps': List of adaptation step counts
        - 'mean_costs': Mean cost at each adaptation step
        - 'std_costs': Std cost at each adaptation step
        - 'regrets': Regret at each adaptation step relative to optimal
        - 'baseline_regrets': Regret relative to baseline policy
        - 'optimal_cost': Optimal cost for this task
        - 'baseline_cost': Baseline cost for this task
        """
        # Get optimal cost for regret calculation
        try:
            K_opt = test_env.get_optimal_K()
            _, _, opt_costs = test_env.rollout(K_opt, steps=self.rollout_length)
            optimal_cost = np.mean(opt_costs)
        except:
            optimal_cost = None

        # Get baseline cost if provided
        baseline_cost = None
        if baseline_policy is not None:
            try:
                _, _, baseline_costs = test_env.rollout(baseline_policy, steps=self.rollout_length)
        ...
        mean_costs = []
        std_costs = []
        regrets = []  # Regret relative to optimal
        baseline_regrets = []  # Regret relative to baseline

        # Evaluate at each adaptation step
        for step in range(max_adaptation_steps + 1):  # Include 0-shot (no adaptation)
            if step == 0:
                # Zero-shot: Use meta-initialization directly
                K_current = self.K.copy()
            else:
                # Few-shot: Adapt for 'step' steps
                K_current = self.adapt_to_task(test_env, num_adaptation_steps=step)

            # Evaluate current policy
            step_costs = []
            for _ in range(num_evaluations):
                try:
                    _, _, costs = test_env.rollout(K_current, steps=self.rollout_length)
                    step_costs.append(np.mean(costs))
                except:
                    continue
            ...
            if step_costs:
                ...
                # Calculate regret relative to optimal
                if optimal_cost is not None and optimal_cost > 0:
                    regret = mean_cost - optimal_cost  # Absolute regret
                    regrets.append(regret)
                else:
                    regrets.append(float('inf'))

                # Calculate regret relative to baseline
                if baseline_cost is not None:
                    baseline_regret = mean_cost - baseline_cost
                    baseline_regrets.append(baseline_regret)
                else:
                    baseline_regrets.append(0.0)
    ...
    def evaluate_meta_learning(self, test_tasks: List, adaptation_steps: List[int] = [0, 1, 3, 5],
                              num_evaluations: int = 5, baseline_policy: np.ndarray = None) -> Dict[str, Any]:
        """
        Evaluate meta-learning performance on multiple unseen tasks.

        Parameters:
        test_tasks: List of unseen test environments
        adaptation_steps: List of adaptation step counts to evaluate
        num_evaluations: Number of evaluations per task per adaptation step
        baseline_policy: Common baseline policy for fair comparison

        Returns:
        Dict with meta-learning evaluation results:
        - 'results_by_steps': Dict mapping adaptation steps to performance metrics
        - 'adaptation_curves': List of adaptation curves for each task
        - 'summary_stats': Overall summary statistics
        """
        # Collect adaptation curves for all test tasks
        adaptation_curves = []
        for test_env in test_tasks:
            curve = self.evaluate_adaptation_curve(
                test_env, max_adaptation_steps=max(adaptation_steps),
                num_evaluations=num_evaluations, baseline_policy=baseline_policy
            )
            adaptation_curves.append(curve)

        # Aggregate results by adaptation steps
        results_by_steps = {}

        for target_steps in adaptation_steps:
            step_costs = []
            step_regrets = []
            step_baseline_regrets = []

            for curve in adaptation_curves:
                # Find closest adaptation step in the curve
                if target_steps in curve['adaptation_steps']:
                    idx = curve['adaptation_steps'].index(target_steps)
                    step_costs.append(curve['mean_costs'][idx])
                    step_regrets.append(curve['regrets'][idx])
                    step_baseline_regrets.append(curve['baseline_regrets'][idx])

            # Compute statistics for this adaptation step
            if step_costs:
                valid_regrets = [r for r in step_regrets if not np.isinf(r)]
                results_by_steps[target_steps] = {
                    'mean_cost': np.mean(step_costs),
                    'std_cost': np.std(step_costs),
                    'mean_regret': np.mean(valid_regrets) if valid_regrets else float('inf'),
                    'std_regret': np.std(valid_regrets) if valid_regrets else float('inf'),
                    'mean_baseline_regret': np.mean(step_baseline_regrets),
                    'std_baseline_regret': np.std(step_baseline_regrets),
                    'num_tasks': len(step_costs)
                }

        # Compute summary statistics
        if adaptation_curves:
            zero_shot_costs = [curve['mean_costs'][0] for curve in adaptation_curves if curve['mean_costs']]
            zero_shot_regrets = [curve['regrets'][0] for curve in adaptation_curves if curve['regrets'] and not np.isinf(curve['regrets'][0])]
            zero_shot_baseline_regrets = [curve['baseline_regrets'][0] for curve in adaptation_curves if curve['baseline_regrets']]

            final_step_idx = max(adaptation_steps) if adaptation_steps else 0
            final_costs = []
            final_regrets = []
            final_baseline_regrets = []

            for curve in adaptation_curves:
                if len(curve['mean_costs']) > final_step_idx:
                    final_costs.append(curve['mean_costs'][final_step_idx])
                    if len(curve['regrets']) > final_step_idx and not np.isinf(curve['regrets'][final_step_idx]):
                        final_regrets.append(curve['regrets'][final_step_idx])
                    if len(curve['baseline_regrets']) > final_step_idx:
                        final_baseline_regrets.append(curve['baseline_regrets'][final_step_idx])

            # Calculate improvement metrics
            cost_improvement = (np.mean(zero_shot_costs) - np.mean(final_costs)) if (zero_shot_costs and final_costs) else 0
            regret_improvement = (np.mean(zero_shot_regrets) - np.mean(final_regrets)) if (zero_shot_regrets and final_regrets) else 0
            baseline_regret_improvement = (np.mean(zero_shot_baseline_regrets) - np.mean(final_baseline_regrets)) if (zero_shot_baseline_regrets and final_baseline_regrets) else 0

            summary_stats = {
                # Raw performance metrics
                'zero_shot_mean_cost': np.mean(zero_shot_costs) if zero_shot_costs else float('inf'),
                'zero_shot_std_cost': np.std(zero_shot_costs) if zero_shot_costs else float('inf'),
                'final_mean_cost': np.mean(final_costs) if final_costs else float('inf'),
                'final_std_cost': np.std(final_costs) if final_costs else float('inf'),

                # Regret metrics (relative to optimal)
                'zero_shot_mean_regret': np.mean(zero_shot_regrets) if zero_shot_regrets else float('inf'),
                'final_mean_regret': np.mean(final_regrets) if final_regrets else float('inf'),

                # Baseline regret metrics (relative to baseline policy)
                'zero_shot_mean_baseline_regret': np.mean(zero_shot_baseline_regrets) if zero_shot_baseline_regrets else 0,
                'final_mean_baseline_regret': np.mean(final_baseline_regrets) if final_baseline_regrets else 0,

                # Improvement metrics
                'cost_improvement': cost_improvement,
                'regret_improvement': regret_improvement,
                'baseline_regret_improvement': baseline_regret_improvement,

                # Meta info
                'num_test_tasks': len(test_tasks),
                'max_adaptation_steps': final_step_idx
            }
        else:
            summary_stats = {}
    ...
    def evaluate(self, test_envs: List, num_evaluations: int = 10) -> Dict[str, float]:
        """
        Evaluate meta-learned policy on test environments.

        Parameters:
        test_envs: List of test environments
        num_evaluations: Number of evaluations per environment
        ...
        for env in test_envs:
            ...
            K_adapted = self.adapt_to_task(env)
        ...
        results = {
            ...
            'std_cost': np.std(all_costs) if all_costs else float('inf'),
            'mean_adaptation_time': np.mean(adaptation_times),
            'num_successful_evaluations': len(all_costs)
        }
